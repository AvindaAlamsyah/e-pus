...
<head>
    ...
    <!-- Title Page-->
    <title>Data Guru | Digital Library</title>
...
<body class="animsition">
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="au-card">
                                <div class="table-responsive">
                                    <table id="tabel-guru" class="table table-striped table-bordered" style="width:100%">
                                        <thead>
                                            <tr>
                                                <th>Username</th>
                                                <th>Nama</th>
        ...
        <div class="modal-dialog" role="dialog">
            <div class="modal-content">
                ...
                <form id="form_hapus" name="form_hapus" type="POST" enctype="multipart/form-data">
                    <div class="modal-body">
                        <label><strong>Apakah anda yakin akan menghapus data guru yang dipilih?</strong></label>
        ...
        <div class="modal-dialog" role="dialog">
            <div class="modal-content">
                ...
                <form id="form_reset" name="form_reset" type="POST" enctype="multipart/form-data">
                    <div class="modal-body">
                        <label><strong>Apakah anda yakin akan me-reset password dari guru yang dipilih?</strong></label>
    ...
    <script>
        ...
        $('#tabel-guru').DataTable({
            'ajax': {
                'url': "<?php echo base_url('admin/data_guru/ambil_semua_guru') ?>",
                'method': "GET"
            },
            'columns': [{
                    'data': "username"
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        return '<div class="btn-group dropleft">' +
                            ...
                            '<a class="dropdown-item item_edit" href="javascript:void(0)" data-item="' + row.id_guru + '">Edit</a>' +
                            '<a class="dropdown-item item_hapus" href="javascript:void(0)" data-item="' + row.id_guru + '">Hapus</a>' +
                            '<a class="dropdown-item item_reset" href="javascript:void(0)" data-item="' + row.id_guru + '">Reset Password</a>' +
                            '</div>' +
                            '</div>';
                    },
                    'width': "5%",
        ...
        $('form[name="form_tambah"]').validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    url: "<?php echo base_url('admin/data_guru/tambah_guru') ?>",
                    type: "POST",
                    dataType: "JSON",
                    data: $(form).serialize(),
        ...
        $('#data-guru').on('click', '.item_edit', function() {
            $("#modal_loading").modal("show");
            $.ajax({
                url: "<?php echo base_url('admin/data_guru/tampil_edit'); ?>",
                type: "POST",
                dataType: "JSON",
                data: {
                ...
                success: function(data) {
                    ...
                    $('#edit_nama').val(data.nama_guru);
                    $('#edit_id').val(data.id_guru);
        ...
        $('form[name="form_edit"]').validate({
            ...
            submitHandler: function(form) {
                $('#modal_loading').modal('show');
                $.ajax({
                    url: "<?php echo base_url('admin/data_guru/simpan_edit') ?>",
                    type: "POST",
                    dataType: "JSON",
                    data: $(form).serialize(),
                    success: function(response) {
        ...
        $('#data-guru').on('click', '.item_hapus', function() {
        ...
        $('#btn_hapus').on('click', function() {
            $('#modal_loading').modal('show');
            $.ajax({
                url: "<?php echo base_url('admin/data_guru/hapus_guru') ?>",
                type: "POST",
                dataType: "JSON",
                data: {
        ...
        $('#data-guru').on('click', '.item_reset', function() {
            let id = $(this).data('item');
            $('#reset_id').val(id);
        ...
        $('#btn_reset').on('click', function() {
            $('#modal_loading').modal('show');
            $.ajax({
                url: "<?php echo base_url('admin/data_guru/reset_password') ?>",
                type: "POST",
                dataType: "JSON",
                data: {
